...
Bu projede aktif skill'ler otomatik tetiklenir — her oturumda listeyi okumana gerek yok.
Yeni bir skill geldiğinde aşağıya bir satır ekle: ne zaman tetiklenir, sana ne kazandırır.

| Skill / Komut | Tetikleyici | Sana ne kazandırır |
|---------------|-------------|---------------------|
| `/write-spec` | Bir feature'yi belgelemek, PRD yazmak istediğinde | Belirsiz fikirden yapılandırılmış spec — hedef, kapsam dışı, metrikler, kabul kriterleri |
| `/competitive-brief` | Rakip analizi gerektiğinde veya farklılaşma kararı verirken | Feature matrisi + diferansiyasyon önerileri; investor/board materyali için de işe yarar |
| `/roadmap-update` | Yeni bilgi gelince öncelik sırası değiştiğinde | Roadmap'i güncel tutar, "ne erteledik, neden?" sorusunu yanıtlar |
| `/brainstorm` | Bir fikri netleştirmek veya seçenekleri keşfetmek istediğinde | Düşünce ortağı — varsayımları sorgular, alternatifleri ortaya koyar |
| `/sprint-planning` | Sprint başlarken kapsam ve kapasite belirlerken | P0 vs. esnek iş ayrımı, carryover kararları, net sprint hedefi |
| `/stakeholder-update` | Yatırımcı, ekip veya müşteri için durum güncelleme yazarken | Kitleye göre uyarlanmış (yönetim özeti / mühendislik detayı / müşteri bildirimi) |
| `/synthesize-research` | Görüşme notu, anket veya geri bildirim yığını analiz ederken | Tema çıkarımı + frekans/etki sıralaması → roadmap önerisi |
| `/metrics-review` | Metrik düşüşü/artışı araştırırken veya dönem sonu incelemesinde | Trend analizi + somut aksiyon önerileri |
| `/feature-start` | Yeni bir iş parçası (feature/bug/araştırma) başlatırken | Oturumlar arası bağlam sürekliliği — kararlar, bulgular, notlar kaybolmaz |
| `skill-creator` | Yeni slash komut yaratmak veya mevcut skill'i geliştirmek istediğinde | Projeye özel otomasyon — tekrarlayan iş akışlarını komuta dönüştürür |
| `youtube-haftalik-ozet` | YouTube kanal performansını analiz ederken | Otomatik rapor — video metrikleri, sentiment, SWOT (Türkçe/İngilizce) |

> Pasif plugin'leri görmek veya etkinleştirmek için: [doc/skill-set.md](../doc/skill-set.md)
