...
2026-05-28 03:15 [gotcha] Android applicationId is `org.betech.fitnes` (NOT `org.betech.fitnes.android` as several historical notes said) — use the short form for adb deeplink commands; the `.android` suffix is the gradle module name, not the package
2026-05-28 03:15 [criteria] Localization invariant: 385 override entries per locale; AZ/RU/EN sets MUST stay identical (Python diff in Phase 6 is the canonical check). Add a key to one locale → add to all three or the build still compiles but the locale silently degrades
2026-05-28 03:15 [refs] Phase 6 deeplink smoke route ↔ devScreen mapping verified working: splash / languageselect / welcome / q1goal / login / authgate (all launched in 1.7–2.2s)
